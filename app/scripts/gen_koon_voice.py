"""
Pre-cache giọng KOON (Trò 1 — Cầu Vồng) bằng edge-tts (giọng vi-VN-HoaiMyNeural).
Chạy:  python app/scripts/gen_koon_voice.py
Yêu cầu:  pip install edge-tts   (không cần API key, miễn phí)

Output: app/assets/audio/koon/<key>.mp3  — phát local tại Gala (không cần mạng).
"""
import asyncio
import os
import edge_tts

VOICE = os.environ.get("KOON_VOICE", "vi-VN-HoaiMyNeural")  # nữ, thân thiện
RATE = os.environ.get("KOON_RATE", "-5%")  # hơi chậm để trẻ dễ nghe
...
async def gen():
    ...
    for key, text in LINES.items():
        text = text.replace("KOON", "Cun")  # tên nhân vật phát âm là "Cun" (KOON chỉ là cách viết)
        ...
        try:
            ...
            print(f"FAIL {key}: {e}")
            fail += 1
    print(f"\nXong: {ok} OK, {fail} FAIL — output: {OUT}")
